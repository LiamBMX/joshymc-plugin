...
**v1.0.49 catalog cleanup (issue #708)**: the old RPG loot catalog (crafting-material weapons/tools/armor sets, consumables, legendary items, `void_bore`/`void_bore_5x5`/`void_bore_chunk`, `carrot_sword`, Bunny armor) was removed — those classes, recipes, resourcepack assets, and the `bunny` kit are gone. Surviving custom items: Void Drill (`void_drill`, `void_drill_5x5`), all eggs (`easter_egg`, `explosive_egg`, `freeze_egg`, `blindness_egg`, `teleport_egg`, `levitation_egg`, `knockback_egg`, `swap_egg`, `lightning_egg`, `cobweb_egg`, `confusion_egg`, `ender_egg`), the September Autumn collection (`autumns_edge`, `harvest_scythe`, `orchard_pickaxe`, `golden_crest`, `falling_leaf`), the October Halloween collection (`phantoms_grasp`, `gravedigger`, `jack_o_lantern_mask`, `bone_rattler`), `bubble_butt_leggings`, ModMode/TraineeMode staff tools (`item/impl/ModModeTools.kt`, `item/impl/TraineeModeTools.kt`), and three items kept as exceptions because a whole command/manager depends on them rather than them being catalog decoration: `afk_key` (AFKManager), `sell_wand` (SellWandListener/SellWandCommand), `token` (`/tokens` economy). `CraftingMaterials.kt` now only defines `soul_fragment` / `ancient_rune` / `enchanted_dust` — kept solely as the crafting ingredients for the two enchant-scroll recipes (`ENCHANT_DUST_SCROLL_RECIPE`, `BEDROCK_BREAKER_SCROLL_RECIPE` in `CustomRecipes.kt`), since scrolls are the only player-facing way to apply a custom enchant.
